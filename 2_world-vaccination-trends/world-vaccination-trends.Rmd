...
filter(world_vaccination, !is.na(pct_vaccinated))
...
```{r}
world_vacc_plot <- world_vaccination %>% 
  ggplot(aes(x = yr, y =pct_vaccinated, colour = vaccine))+
  geom_point()+
  ggtitle("Anything")+
  xlab("year")+
  ylab("percentage of people vaccinated")
world_vacc_plot
```
...
```{r}
plot <- world_vaccination %>% 
  ggplot(aes(x = yr, y = pct_vaccinated, colour = who_region))+
  geom_line()+
  facet_wrap(~vaccine)+
  ggtitle("Comparison between polio and hepatitis B vaccines")+
  xlab("year")+
  ylab("percentage of people vaccinated")+
  labs(colour = "WHO regions")
